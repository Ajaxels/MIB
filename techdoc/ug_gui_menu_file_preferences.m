...
%
% *Please note*, |im_browser| stores its configuration parameters in a file that is automatically generated after closing of
% |im_browser|:
%
% * *for Windows* - _c:\temp\im_browser.mat_ or when _c:\temp_ is unavailable in the Windows TEMP directory (_C:\Users\User-name\AppData\Local\Temp\_). 
% The TEMP directory can be found and accessed with |Windows->Start button->%TEMP%| command
% * *for Linux* - in a directory where |im_browser| is installed, or in the local |tmp| directory (_/tmp_)
%
% 
...
%% Preferences
% 
% <<images\menu_file_preferences.jpg>>
% 
%
...
% # *Number of points* - number of points to use for the interpolation, more points give smoother results, white the less points is faster.
% # *Line width* - defines width of the line after the interpolation.
%
%% Models panel
% The models can be encoded in two ways:
% 
% * *63 materials*, _default_, allows to store Models, Selection and Mask layers in a single memory container. Reduce memory requirements
% but limits the number of materials to 63.
% * *255 materials*, allows to have up to 255 materials in the model, but requires additional computer memory to keep
% Selection and Mask layers, which means that for |uint8| class of the datasets memory requirements are doubled.
%
%% Annotations panel
% Models may include annotations. The annotations are strings of text bound
% to certain coordinates within the dataset. Annotations may be accessed
% from the menu entry: |Menu->Models->Annotations|.
% 
...
% <tr>
% <td style="border: 0px">
%   <img src = "images\menu_file_preferences_color_popup.jpg">
% </td>
% <td style="border: 0px">
% <ul>
...
%
% 
% <<images\menu_file_preferences_color_brewer.jpg>>
% 
...
% It is possible to modify the font size and font name for the MIB. Please
% use one of the following options:
%
% * *Select Font button* to start a standard font selection dialog
% * *Font size edit box*, to change default font size for all text panels
% * *Font size for dir contents*, to change the font size for the list of
