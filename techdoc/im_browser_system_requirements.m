...
%
% *<http://www.loci.wisc.edu/software/bio-formats LOCI> Bio Formats* java library (_optional_) is provided in the
% |FILE_FORMATS/BioFormats| folder.
%
...
%
%% DipLib library
% When DipLib is installed, Microscopy Image Browser can use several additional methods for anisotropic
% diffusion filtering available from <http://www.diplib.org DipLib>.
% Please refer to the following step for installation.
%
% * Download diplib from <http://www.diplib.org/download
% www.diplib.org/download> (free for non-commercial use)
% * Install the package, For example, start |DIPimage 2.3 installer win 64.exe| for
% automatic installation on Windows 64-bit operating system
% * Start Matlab
% * Add directory where the |diplib/dipimage| was installed into the Matlab path: |Matlab->Home tab->Set path...->Add
% folder...->Move to Bottom->Save|. For example, |c:\Program Files\DIPimage 2.3\common\dipimage\|
% 
...
% * Start Matlab
% * Add Fiji.app/scripts folder to the Matlab path and save the path: |Matlab->Home tab->Set path...->Add
% folder...->Move to Bottom->Save|. For example, |c:\Program
% Files\Fiji.app\scripts\|. *For Mac OS* use the addpath command from the
...
% 
% Please note, the connection is not available in the compiled version of
% MIB.
% 
% *Requirements:*
%
