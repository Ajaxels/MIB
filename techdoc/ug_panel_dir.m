...
%
% 
% <<images\panel_dir_buffermenu.jpg>>
% 
%%
% 
% * *Duplicate dataset*, creates a duplicate of the currently shown dataset and put it behind one of the available buttons.
...
% list of files in the selected directory (the directory can be selected from the <ug_panel_path.html Path Panel>). The files are filtered with the
% specified filter (*3*). Value |all known| displays all readable formats.
%%
% 
% * Double click on a file, loads it into |im_browser| and displays it in the
...
%
% 
% <<images\panel_dir_filecontext.jpg>>
% 
% <html>
...
% * <ug_panel_corr.html Correlation Analysis>
% * <ug_panel_bg_removal.html Background Removal>
% * <ug_panel_fiji_connect.html Fiji Connect>
%
%
%% 7. Bio checkbox
% |im_browser| can also open some specific biological image
% formats. Usually these are the native file formats for different
% microscopes. The reader for these formats is obtained from 
% <http://www.loci.wisc.edu/software/bio-formats LOCI Bio-formats>
% and is based on connected to |im_browser| LOCI Java library (the library is stored in the 
% |File_formats| directory). When the |Bio| checkbox is selected the LOCI reader is called to select a desired dataset.
%
%% 8. Z-stack checkbox
% When checked |im_browser| combines all shown in the list box
% (*2*) files into a single Z-stack. 
%
%% 9. ?.
