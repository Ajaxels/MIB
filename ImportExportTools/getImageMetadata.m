...
if options.waitbar==1; wb = waitbar(0,'Loading metadata','Name','Metadata import'); end
...
for fn_index = 1:no_files
    ...
    elseif strfind('xml', ext(2:end)) > 0 & options.bioformatsCheck == 0     %#ok<AND2>
...
%         end
    ...
    elseif numel(strfind('nrrd', ext(2:end))) > 0 && options.bioformatsCheck == 0     % NRRD format
        files(fn_index).filename = cell2mat(filenames(fn_index));
    ...
    elseif numel(strfind('am', ext(2:end))) > 0 && options.bioformatsCheck == 0     % Amira Mesh
        files(fn_index).filename = cell2mat(filenames(fn_index));
        ...
        if ~isempty(bbStart)
            ...
            if isempty(brakePnt)
                brakePnt = strfind(info('ImageDescription'), sprintf('\t'));
                if isempty(brakePnt)
                    brakePnt = numel(info('ImageDescription'));
                end
            end
            brakePnt = brakePnt(1);
    ...
    elseif strfind('recmrcstpreali', ext(2:end)) > 0 & options.bioformatsCheck == 0         %#ok<AND2> % MRC format
        files(fn_index).filename = cell2mat(filenames(fn_index));
...
%             if max(filesTemp.dim_xyczt(3)) > 1
%                 currentColorType = 'truecolor';
%             else
%                 currentColorType = 'grayscale';
%             end
...
%             end
            ...
            %if numel(filesTemp.seriesIndex) > 1  % generate filename with the series number
