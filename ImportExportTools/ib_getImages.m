function [img, img_info] = ib_getImages(files, img_info, options)
% function [img, img_info] = ib_getImages(files, img_info, options)
...
if options.waitbar; wb = waitbar(0, 'Please wait...', 'Name', 'Loading images...'); end;
for fn_index = 1:no_files
    ...
    if strcmp(files(fn_index).object_type, 'movie')      % movie object
        ...
        % img(:,:,:,layer_id:layer_id+files(fn_index).noLayers-1) = read(xyloObj);
    ...
    elseif strcmp(files(fn_index).object_type,'hdf5_image') || strcmp(files(fn_index).object_type,'matlab.hdf5')        % HDF5 format
        ...
        if isa(hdf5_image,'single')   % convert to unsigned integers
            ...
            msgbox(sprintf('The dataset was converted to %s format!',class(hdf5_image)),'Warning!','warn','modal');
        end
        ...
        %img(1:maxY,1:maxX,1:maxC,layer_id:layer_id+files(fn_index).noLayers-1,1:maxT) = hdf5_image(1:maxY,1:maxX,1:maxC,1:files(fn_index).noLayers);
...
%                 end
    ...
    elseif strcmp(files(fn_index).object_type,'mrc_image')        % IMOD mrc/rec image
        ...
        if isa(mrc_image,'single') || isa(mrc_image,'int16')
            ...
            if minInt < 0
                button =  questdlg('The dataset will be converted to unsigned integer class...','Convert image','Yes','Cancel','Yes');
    ...
    elseif strcmp(files(fn_index).object_type,'bioformats')        % bioformats images
        for serieIndex=1:numel(files(fn_index).seriesName)
            ...
            for subLayer=1:files(fn_index).noLayers        % filling image Info structure
                img(1:maxY,1:maxX,1:maxC,layer_id) = I.img(1:maxY,1:maxX,1:maxC,subLayer);
