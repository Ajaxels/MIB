...
if ~isfield(handles, 'vImarisApp')
    vImarisLib = ImarisLib;
    vObjectId = 0; 
    handles.vImarisApp = vImarisLib.GetApplication(vObjectId);
...
vImage = handles.vImarisApp.GetDataSet();   % get handle to a dataset; class IDataSet
if isequal(vImage, [])
	display('ib_getImarisDataset: error! a volume should be opened in Imaris')
    return;
end
