function handles = ib_anisotropicDiffusion(handles, filter_type)
% function handles = ib_anisotropicDiffusion(handles, filter_type)
...
switchIndexed = get(handles.menuImageIndexed,'Checked');
...
if strcmp(filter_type,'diplib') || strcmp(filter_type, 'anisodiff')    % do diplib or anisodiff filtering
    ...
    opt = get(handles.imageFilterPopup,'String');
    if ischar(opt)
        sel_filter = opt;
    else
        sel_filter = opt{val};
    end
    options.Iter = str2double(get(handles.imfiltPar1Edit,'String'));
    options.KSigma = str2double(get(handles.imfiltPar2Edit,'String'));
    options.Lambda = str2double(get(handles.imfiltPar3Edit,'String'));
    options.Orientation = handles.Img{handles.Id}.I.orientation;
    options.Color = colorChannel;
    options.Favours = get(handles.andiffPopupFavours, 'value'); % 1: favours high contrast edges over low contrast ones; 2: favours wide regions over smaller ones. For 'anisodiff' only
    switch sel_filter
        case 'Perona Malik anisotropic diffusion'; 
            options.Filter = 'anisodiff'; 
            options.KSigma = options.KSigma/100*double(intmax(class(handles.Img{handles.Id}.I.img)));   % convert K from %% to numbers
    ...
    options.start_no = handles.Img{handles.Id}.I.getCurrentSliceNumber();
    ...
    for t=timeVector(1):timeVector(2)
        if ~strcmp(mode, '2D, shown slice')
            img = ib_getStack('image', handles, t, 0, options.Color);
            options.start_no = 1;
            options.end_no = size(img{1},4);
        else
            getDataOptions.t = [t t];
            img = ib_getSlice('image', handles, handles.Img{handles.Id}.I.getCurrentSliceNumber(), NaN, options.Color, getDataOptions);
            options.start_no = 1;
            options.end_no = 1;
        end
        ...
        switch doAfter
            case 'Apply filter'
                [img, status] = do_andiff_filtering(img, options);
                if status == 0; return; end;
            case 'Apply and add to the image'
                [imgOut, status] = do_andiff_filtering(img, options);
                if status == 0; return; end;
                for roi = 1:numel(img)
        ...
        if ~strcmp(mode, '2D, shown slice')
            ib_setStack('image', img, handles, t, 0, options.Color);
        else
            ib_setSlice('image', img, handles, handles.Img{handles.Id}.I.getCurrentSliceNumber(), NaN, options.Color, getDataOptions);
        end
