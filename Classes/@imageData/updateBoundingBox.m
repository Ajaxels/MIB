...
% imageData.pixSize.units.
% imgDims: [@em optional] a vector [height, width, z-stacks] with dimensions of the image; when omitted the function will use
% imageData.height, imageData.width and imageData.no_stacks variables. @b Note! the units here are the image units:
% imageData.pixSize.units.
%
% Return values:
...
% @code imageData.updateBoundingBox(NaN, xyzShift);  // get bounding box info @endcode
% @code updateBoundingBox(obj, NaN, xyzShift);   // Call within the class; get bounding box info @endcode
% @code newBB = [15 50 10 150 1 15];  // define new bounding box in imageData.pixSize.units @endcode
% @code imageData.updateBoundingBox(newBB);  // get bounding box info @endcode

% Copyright (C) 18.08.2014, Ilya Belevich, University of Helsinki (ilya.belevich @ helsinki.fi)
% 
...
if nargin < 4
    h = obj.height;
    w = obj.width;
    no_stacks = obj.no_stacks; %#ok<PROP>
else
    h = imgDims(1);
    w = imgDims(2);
    no_stacks = imgDims(3);     %#ok<PROP>
end
if nargin < 2
    newBB = NaN;
end;
...
if isnan(newBB)     % shift the existing bounding box
    ...
    dz = (max([no_stacks 2])-1)*obj.pixSize.z*coef; %#ok<PROP>
    %             dx = (max([w 1]))*obj.pixSize.x*coef;
    %             dy = (max([h 1]))*obj.pixSize.y*coef;
    %             dz = (max([no_stacks 1]))*obj.pixSize.z*coef; %#ok<PROP>
...
if bb_info_exist == 1
    ...
    obj.img_info('ImageDescription') = [str2 curr_text(pos:end)];
else
    obj.img_info('ImageDescription') = [str2 curr_text];
end

obj.pixSize.x = (newBB(2)-newBB(1))/(w-1);
obj.pixSize.y = (newBB(4)-newBB(3))/(h-1);
obj.pixSize.z = (newBB(6)-newBB(5))/(max([no_stacks-1, 1]));
end