...
%| 
% @b Examples:
% @code handles = imageData.swapColorChannels(1, 3);     // swap color channel 1 and 3 @endcode
% @code handles = swapColorChannels(obj, 1, 3); // Call within the class; swap color channel 1 and 3 @endcode
...
wb = waitbar(0,sprintf('Swap color channels %d and %d\n\nPlease wait...',channel1, channel2),'Name','Swap color channels','WindowStyle','modal');
dummy = obj.img(:,:,channel1,:,:);
waitbar(0.33, wb);
obj.img(:,:,channel1,:,:) = obj.img(:,:,channel2,:,:);
waitbar(0.66, wb);
obj.img(:,:,channel2,:,:) = dummy;
