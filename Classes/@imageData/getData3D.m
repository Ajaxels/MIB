...
%| 
% @b Examples:
% @code dataset = handles.Img{handles.Id}.I.getData3D('image');      // get the 4D dataset for the current time point, in the shown orientation  @endcode
% @code dataset = handles.Img{handles.Id}.I..getData3D('image', 5, 4, 2); // get the 4D dataset for the 5-th time point in the XY orientation @endcode
% @code dataset = handles.Img{handles.Id}.I..getData3D('selection', 5, 1, 2, custom_img); // get the 5-th timepoint in the the XZ-orientation, color channel=2 @endcode
% @code dataset = obj.getData3D(img, 'image', [], 4);      // Call within the class; get the 4D dataset for the current time point in the XY orientation  @endcode
...
dataset = obj.getData4D(type, orient, col_channel, options, custom_img);
end